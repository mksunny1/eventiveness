...
eventivity.HandlerContext = class extends eventivity.Context {
    ...
    create(handlerFunction, options) {
        ...
        if (handlerFunction) {
            ...
            let handler;
            if (this.options?.owner) handler = [this, handlerFunction, handlerName, new WeakRef(this.options?.owner)];
            else handler = [this, handlerFunction, handlerName];
...
eventivity.EventContext = class extends eventivity.Context {
    ...
    handle(handler, handlersArg, event, eventHandlers) {
        ...
        if (this.options?.clear || handlerContext.options?.clear) delete eventHandlers[handlerName];
        return result;
    };
    dispatch(handlersArg) {
        ...
        let eventHandlers, values, handler, result;
