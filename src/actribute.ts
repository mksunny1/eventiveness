...
export class Actribute {
    ...
    process(element: HTMLElement, props: any): Actribute {
        let compProps: any[] = [], comp, propKey, propVal, foundAllProps, processed = false;
        ...
        if (!processed) for (let child of Array.from(element.children)) this.process(element, props);
        return this;
    }
}
