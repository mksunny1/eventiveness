...
const oneTrap = {
    get(target: One, p: typeof PURE) {
        ...
        if (result.length && typeof result[0] === 'function') {
            ...
            if (target.ctor) return target.ctor(result, true, target.context, target.ctor);
...
export class One {
    ...
    get(prop?: string | number | symbol | null, forceArray?: boolean): any[]|One {
        ...
        const args: [any[], boolean|undefined, any|undefined] = [results, this.recursive, this.context];
    ...
    set(prop?: string | number | symbol | null, values?: any[]) {
        if (values === undefined) return this.set(prop, this.get(prop, true) as any[]);  
