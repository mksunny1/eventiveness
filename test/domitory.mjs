...
import {insert, inserter, set, update, createRange, remove} from '../src/domitory.js'
...
Object.assign(window, {insert, inserter, set, update, createRange, remove} , 
    { createFragment, arrayTemplate }, {apply, range, report, items});

window.expectedBodyContent = await fetch('./domitory.json').then(r => r.json());
window.bodyContent = [];
...
// 1. insert
insert([document.body], [createFragment(`
    <h1>Title</h1>
...
apply({
    main: main => {
        main.innerHTML = '';
        insert([main], [createFragment(arrayTemplate(`
            <article>
                <h2>Article \${item}</h2>
                <p>No body for now</p>
            </article>
...
apply({
    main: main => {
        const newElements = createFragment(arrayTemplate(`
        <p>
...
apply({
    main: main => {
        const newContent = items([...range(101, 120)].map(i => 'background-color: navy; color: white'), range(5, 15, 2));
        const firstChildren = items(Array.from(main.children).map(c => c.firstElementChild), range(5, 15, 2));
...
apply({
    main: main => {
        let newElements = createFragment(arrayTemplate(`
        <h2 style="background-color: yellow;">
            Now this is a new title \${item}
        </h2>
        `)(range(1, 20))).children;
        newElements = items(newElements, range(0, 7));
        const firstChildren = items(Array.from(main.children).map(c => c.firstElementChild), range(7));
...
apply({
    main: main => {
        const lastChildren = items(Array.from(main.children).map(c => c.lastElementChild), range(7, 17));
        remove(lastChildren);
    }
});
...
for (let i = 0; i < window.bodyContent.length; i++) {
    report(window.bodyContent[i][1], expectedBodyContent[i][0], window.bodyContent[i][0]);
}
